...
const BookingForm = ({ 
  ...
  const [formData, setFormData] = useState({
    ...
    customer_name: mode === 'contractor' ? user?.full_name || user?.company_name || '' : '',
    customer_email: mode === 'contractor' ? user?.email || '' : '',
    customer_phone: mode === 'contractor' ? user?.phone || '' : '',
    location_address: '',
    location_city: '',
    location_state: '',
  ...
  // Update contractor info when user context changes
  React.useEffect(() => {
    if (mode === 'contractor' && user) {
      setFormData(prev => ({
        ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    // For contractor mode, ensure user info is available
    if (mode === 'contractor') {
      if (!user?.email) {
        alert('Your email is required. Please update your profile.');
        return;
      }
    }
    ...
    try {
      ...
      const requestData = {
        company_id: mode === 'contractor' ? user.company_id : null,
        // Don't set customer_id for contractor-posted requests - let it be null
        ...
        // Note: customer_id left null for contractor-posted requests
      };
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                <label htmlFor="customer_name" className="block text-sm font-medium text-gray-700">
                  {mode === 'contractor' ? 'Your Name' : 'Full Name'} *
                </label>
                <input
                  type="text"
                  id="customer_name"
                  ...
                  onChange={handleChange}
                  readOnly={mode === 'contractor'}
                  className={`mt-1 focus:ring-primary-500 focus:border-primary-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md ${
                    mode === 'contractor' ? 'bg-gray-50 text-gray-600' : ''
                  }`}
                  required
                />
              </div>
              ...
              <div>
                <label htmlFor="customer_phone" className="block text-sm font-medium text-gray-700">
                  {mode === 'contractor' ? 'Your Phone' : 'Phone Number'}
                </label>
                <input
                  type="tel"
                  id="customer_phone"
            ...
            <div>
              <label htmlFor="customer_email" className="block text-sm font-medium text-gray-700">
                {mode === 'contractor' ? 'Your Email' : 'Email Address'} *
              </label>
              <input
                type="email"
                id="customer_email"
