...
const MessageThread = ({ contextType, contextId, title }) => {
  ...
  useEffect(() => {
    if (customer?.customer_id && contextId) {
      loadMessages();
    }
  }, [customer, contextId]);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      if (contextType === 'request') {
        recipientInfo = await MessagingService.getContractorForRequest(contextId);
        ...
        recipientInfo = await MessagingService.getContractorForJob(contextId);
        ...
        <div className="space-y-4 mb-6" style={{ maxHeight: '400px', overflowY: 'auto' }}>
          {messages.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-gray-500 mt-1">Start a conversation with the contractor</p>
            </div>
          ) : (
