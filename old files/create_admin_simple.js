...
/**
 * Simple TradeMate Pro Admin User Creator
 * 
 * This script creates the first admin user for your TradeMate Pro admin dashboard.
 * Uses a simpler approach with hardcoded values for testing.
 */
...
// Hardcoded values for testing - change these as needed
...
async function createAdminUser() {
  console.log('\n🔧 TradeMate Pro Simple Admin Creator');
  console.log('=====================================\n');
  ...
  try {
    ...
    console.log('3️⃣ Creating user record with admin role...');
    const { data: userRecord, error: userError } = await supabase
      .from('users')
      .insert({
        auth_user_id: authUser.user.id,
        role: 'admin',
        company_id: null, // Admin doesn't belong to a specific company
        status: 'active'
      })
      .select()
      .single();
    ...
    console.log('✅ User record created with admin role:', userRecord.id);
    
    // Step 4: Verify the setup
    console.log('4️⃣ Verifying admin setup...');
    const { data: verifyUser, error: verifyError } = await supabase
      ...
      .select(`
        ...
        company_id,
        status,
        profiles!users_auth_user_id_fkey (
          first_name,
          last_name,
          user_id
        )
...
// Run the script
console.log('🔧 Initializing TradeMate Pro Simple Admin Creator...');
